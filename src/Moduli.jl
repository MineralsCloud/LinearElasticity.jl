...
BulkModulus(λ::Lame1stParameter, G::ShearModulus) = BulkModulus(λ.v + 2 * G.v / 3)
BulkModulus(λ::Lame1stParameter, ν::PoissonRatio) = BulkModulus(λ.v * (1 + ν.v) / 3 / ν.v)
BulkModulus(λ::Lame1stParameter, M::LongitudinalModulus) = BulkModulus((M.v + 2 * λ.v) / 3)
...
YoungModulus(K::BulkModulus, λ::Lame1stParameter) =
...
YoungModulus(λ::Lame1stParameter, G::ShearModulus) =
    YoungModulus(G.v * (3 * λ.v + 2 * G.v) / (λ.v + G.v))
YoungModulus(λ::Lame1stParameter, ν::PoissonRatio) =
    YoungModulus(λ.v * (1 + ν.v) * (1 - 2 * ν.v) / ν.v)
YoungModulus(λ::Lame1stParameter, M::LongitudinalModulus) =
...
_auxiliaryR(E::YoungModulus, λ::Lame1stParameter) = sqrt(E.v^2 + 9 * λ.v^2 + 2 * E.v * λ.v)
